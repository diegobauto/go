...
import (
	...
	"github.com/udistrital/agenda_api/models"
...
func (c *ContactoController) Post() {
	...
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if _, err := models.AddContacto(&v); err == nil {
			...
			logs.Error(err)
			c.Data["message"] = "Error service POST: The request contains an incorrect data type or and invalid parameter"
			c.Abort("400")
		}
	} else {
		logs.Error(err)
...
func (c *ContactoController) GetOne() {
	...
	if err != nil {
		logs.Error(err)
		c.Data["message"] = "Error service GeOne: The request contains an incorrect parameter or no record exists"
		c.Abort("404")
	} else {
...
func (c *ContactoController) GetAll() {
	...
	if err != nil {
		logs.Error(err)
		c.Data["message"] = "Error service GeAll: The request contains an incorrect parameter or no record exists"
		c.Abort("404")
	} else {
...
func (c *ContactoController) Put() {
	...
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		...
		v.FechaModificacion = time_bogota.TiempoBogotaFormato()
		fmt.Println("MODELO put:", v)
		if err := models.UpdateContactoById(&v); err == nil {
			c.Data["json"] = map[string]interface{}{"Success": true, "Status": "200", "Message": "Update successful", "Data": v}
		} else {
			logs.Error(err)
			c.Data["message"] = "Error service Put: The request contains an incorrect data type or and invalid parameter"
			c.Abort("400")
		}
	} else {
		logs.Error(err)
